...
public class SupplierPortImpl implements SupplierPortType {
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		if(descText == null || descText.trim().length() == 0)
			throwBadText("Text is invalid");
		
		List<ProductView> l = new ArrayList<ProductView>();
		Supplier supplier = Supplier.getInstance();
		
		for(String id : supplier.getProductsIDs()){
			Product p = supplier.getProduct(id);
			
			if(p.getDescription().contains(descText))
				l.add(newProductView(p));
		}
	...
	throws BadProductId_Exception, BadQuantity_Exception, InsufficientQuantity_Exception {
		if(productId == null || !productId.matches("[A-Z0-9a-z]+"))
		...
		if(p != null && p.getQuantity() >= quantity){
			try
			{
				res = supplier.buyProduct(productId, quantity);;
			}
			catch(QuantityException qtex){
				throwInsufficientQuantity(qtex.getMessage());
			}
		}
		else throwInsufficientQuantity("Not enough quantity to sell!");
